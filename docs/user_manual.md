...
1. **ITS-OBU (Aplikace v telefonu v2.0.0)** – Fyzicky se nachází v autě, měří sílu signálů, plní roli testovacího klienta a pod obrovskou zátěží (až 100 Mbps) střílí data do laboratoře.
...
2. Vložte **veřejnou IP adresu** sítě, ve které se laboratoř nachází (např. `147.32.103.236`).
...
* KROK 1: Z jakéhokoliv notebooku či telefonu na *datech* (mimo fakultní síť) zkuste v `cmd`/terminálu zapsat `ping 147.32.103.236`. (ICMP mohl univerzitní firewall zablokovat, to je v pořádku, krok 2 ukáže víc).
* KROK 2: Dále napište `curl -v http://147.32.103.236:8000/docs` pro kontrolu API portu. Pokud vypíše "Connection Timed Out", s obrovskou jistotou za to nemůže chyba v kódu serveru, ale **nepropuštěný ochranný firewall sítě**. Zavolejte správce vaší sítě laboratoře.
...
* > [!IMPORTANT]
> **Pozor na reinstalaci:** Pokud aplikaci odinstalujete a znovu nainstalujete, Android smaže vnitřní mezipaměť (SharedPreferences). Tím dojde k resetování nastavené IP adresy na výchozí laboratorní `192.168.0.161`. Před každým terénním testem po nové instalaci tedy nezapomeňte v Nastavení znovu zadat veřejnou IP `147.32.103.236`.
>
